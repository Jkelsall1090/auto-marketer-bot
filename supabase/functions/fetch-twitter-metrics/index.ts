...
async function getTweetMetrics(tweetIds: string[]): Promise<any> {
  ...
  if (!response.ok) {
    ...
    return { data: [], error: responseText, status: response.status };
  }
...
serve(async (req) => {
  ...
  try {
    ...
    let apiLimitedMessage: string | null = null;
    
    if (tweetIds.length > 0 && API_KEY && API_SECRET && ACCESS_TOKEN && ACCESS_TOKEN_SECRET) {
      twitterData = await getTweetMetrics(tweetIds);
      
      // Check if we hit API tier limitation
      if (twitterData.status === 401 || twitterData.status === 403) {
        apiLimitedMessage = 'Twitter API metrics require Basic tier ($100/month). Showing post counts only.';
        console.log(apiLimitedMessage);
      }
    }
        ...
        if (action) {
          ...
          await supabase
            ...
            .update({ 
              ...
              click_count: metrics.impression_count || 0,
            })
            .eq('id', action.id);
        }
      }
    ...
    // Include actions without Twitter API data (using stored engagement counts)
    for (const action of actions) {
    ...
    return new Response(
      JSON.stringify({ 
        ...
        api_limited: !!apiLimitedMessage,
        api_limited_message: apiLimitedMessage,
      }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );
